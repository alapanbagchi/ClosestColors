...
def splitColors(hex):
    # Convert the red part of the hex color from hex to decimal
    r = int(hex[1:3], 16)
    # Convert the green part of the hex color from hex to decimal
    g = int(hex[3:5], 16)
    # Convert the blue part of the hex color from hex to decimal
    b = int(hex[5:7], 16)
    return [r, g, b]
...
def distance(hex1, hex2):
    ...
    # Return the eucledian distance between two hex colors
    return int((((l2 - l1)**2 + (a2 - a1)**2 + (b2 - b1)**2) ** (1/2)))
...
def result():
    ...
    for i in range(len(search)):  # Iterate through the search array
        # Array to store the least distance between colors initialized with a default value
        least.append(distance(search[i], pallete[i]))
        # For storing closest color found from the database. In this line it is initialized with a default value from the pallete which will be change in the next loop if a closer color is found
        least_val = pallete[i]
        for j in range(len(pallete)):  # Iterate through pallete array
            # Check if the distance between two colors is lesser than the value of least[i]
            if(distance(search[i], pallete[j]) < least[i]):
                # Store the least distance in least[i]
                least[i] = distance(search[i], pallete[j])
